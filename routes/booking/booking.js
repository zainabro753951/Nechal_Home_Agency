...
router.post("/booking/:infoId", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    let bookingSuccess = "Your Booking will be successfully sended";
    res.redirect(`/propertyinfo/${infoId}?bookingSuccess=${bookingSuccess}`);
  } catch (error) {
    console.error(error);
